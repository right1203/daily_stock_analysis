# -*- coding: utf-8 -*-
"""PushPlus sender compatibility shim.

PushPlus delivery is out of scope for the KR+US product profile.
"""

import logging
from typing import Any, Optional


logger = logging.getLogger(__name__)


class PushplusSender:
    """No-op sender retained for backward compatibility."""

    def __init__(self, config: Any) -> None:  # noqa: D107
        self._removed = True

    def send_to_pushplus(self, content: str, title: Optional[str] = None) -> bool:
        logger.warning("PushPlus delivery is not supported in the KR+US build.")
        return False

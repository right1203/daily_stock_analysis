...
class BacktestRunRequest(BaseModel):
    code: Optional[str] = Field(None, description="특정 종목만 백테스트합니다. 예: 005930 또는 AAPL")
    force: bool = Field(False, description="기존 결과가 있어도 다시 계산합니다.")
    eval_window_days: Optional[int] = Field(None, ge=1, le=120, description="평가 기간(거래일 기준)")
    min_age_days: Optional[int] = Field(
        None,
        ge=0,
        le=365,
        description="분석 기록의 최소 경과일(0=제한 없음)",
    )
    limit: int = Field(200, ge=1, le=2000, description="처리할 분석 기록의 최대 개수")

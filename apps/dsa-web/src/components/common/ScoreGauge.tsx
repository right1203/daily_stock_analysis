...
/**
 * Sentiment score gauge - glowing circular progress
 * Financial-terminal-inspired design with transition animation
 */
export const ScoreGauge: React.FC<ScoreGaugeProps> = ({
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      // Use the easeOutCubic easing function
  ...
  // Start from the top and display a 270-degree arc (3/4 circle)
  const arcLength = circumference * 0.75;
  const progress = (animatedScore / 100) * arcLength;

  // Color mapping, using the animated score to calculate color transitions
  const getStrokeColor = (s: number) => {
    if (s >= 60) return '#00d4ff'; // Cyan - positive
    if (s >= 40) return '#a855f7'; // Purple - neutral
    return '#ff4466'; // Red - negative
  };

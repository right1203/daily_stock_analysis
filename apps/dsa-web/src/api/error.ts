...
export function parseApiError(error: unknown): ParsedApiError {
  ...
  if (includesAny(matchText, ['agent mode is not enabled', 'agent_mode'])) {
    return createParsedApiError({
      ...
      message: '현재 기능은 Agent 모드가 필요합니다. 먼저 켠 뒤 다시 시도해 주세요.',
      rawMessage,
      status,
  ...
  const hasMissingParamText = includesAny(matchText, ['stock_code or stock_codes required', 'missing', 'required']);
  if (hasStockCodeField && hasMissingParamText) {
    return createParsedApiError({
      title: '요청에 필수 매개변수가 없습니다',
      message: '종목 코드 또는 필요한 입력값을 먼저 입력한 뒤 다시 시도해 주세요.',
      rawMessage,
      status,
  ...
  if (noConfiguredLlm) {
    return createParsedApiError({
      ...
      message: '시스템 설정에서 기본 모델, 사용 가능한 채널 또는 관련 API Key를 먼저 설정한 뒤 다시 시도해 주세요.',
      rawMessage,
      status,
  ...
  if (includesAny(matchText, [
    ...
    return createParsedApiError({
      ...
      message: '현재 모델은 Agent 또는 도구 호출 시나리오에 적합하지 않습니다. 도구 호출을 지원하는 모델로 변경한 뒤 다시 시도해 주세요.',
      rawMessage,
      status,
  ...
  if (includesAny(matchText, [
    ...
    return createParsedApiError({
      title: '상위 모델이 반환한 데이터 구조가 불완전합니다',
      message: '상위 모델의 도구 호출 구조가 요구 사항과 맞지 않습니다. 모델을 변경하거나 관련 추론 모드를 끈 뒤 다시 시도해 주세요.',
      rawMessage,
      status,
  ...
  if (includesAny(matchText, ['timeout', 'timed out', 'read timeout', 'connect timeout']) || code === 'ECONNABORTED') {
    return createParsedApiError({
      ...
      message: '서버가 외부 의존성에 접근하는 중 시간이 초과되었습니다. 잠시 후 다시 시도하거나 현재 네트워크와 프록시 설정을 확인해 주세요.',
      rawMessage,
      status,
  ...
  if (
    ...
    return createParsedApiError({
      ...
      message: '페이지는 로컬 서비스에 연결되었지만 로컬 서비스가 외부 모델 또는 데이터 API에 접근하지 못했습니다. 프록시, DNS 또는 아웃바운드 네트워크 설정을 확인해 주세요.',
      rawMessage,
      status,
  ...
  if ((status === 400 || includesAny(matchText, ['bad request'])) && hasLlmProviderHint) {
    return createParsedApiError({
      ...
      message: '로컬 서비스는 정상이나 상위 모델 API가 요청을 거부했습니다. 모델 이름, 매개변수 형식 또는 도구 호출 호환성을 확인해 주세요.',
      rawMessage,
      status,
  ...
  if (localConnectionFailed) {
    return createParsedApiError({
      ...
      message: '브라우저가 현재 로컬 Web 서비스에 연결할 수 없습니다. 서비스가 실행 중인지, 수신 주소가 올바른지, 포트가 열려 있는지 확인해 주세요.',
      rawMessage,
      status,
  ...
  if (payloadText || status) {
    return createParsedApiError({
      title: '요청 실패',
      message: payloadText ?? `요청을 완료하지 못했습니다(HTTP ${status}).`,
      rawMessage,
      status,
      category: 'http_error',
    });
...
export function toApiErrorMessage(error: unknown, fallback = '요청을 완료하지 못했습니다. 잠시 후 다시 시도해 주세요.'): string {
